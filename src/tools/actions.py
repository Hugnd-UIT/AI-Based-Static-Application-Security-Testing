...
def read_file(tool_args: dict, target_dir: str, ts_module=None) -> str:
    ...
    try:
        abs_path = resolve_abs_path(target_dir, rel_path)
...
def trace_variable(tool_args: dict, target_dir: str, ts_module=None) -> str:
    ...
    try:
        ...
        abs_path = resolve_abs_path(target_dir, file_path)
        ...
        return trace_variable_fallback(var_name, file_path, target_dir)
    except Exception as trace_err:
        return f"[ERROR] trace_variable({var_name}): {trace_err}"

def trace_variable_fallback(var_name: str, file_path: str, target_dir: str) -> str:
    try:
        abs_path = resolve_abs_path(target_dir, file_path)
        with open(abs_path, "r", encoding="utf-8", errors="replace") as file_handle:
            file_lines = file_handle.readlines()
...
def find_function(tool_args: dict, target_dir: str, ts_module=None) -> str:
    ...
    try:
        tree_sitter = ts_module or load_tree_sitter()
        func_source = tree_sitter.get_func_code(target_dir, func_name)
